...
def render_teacher_view(storage) -> None:
    ...
    st.write("Upload one question, then provide several student answers. MathBridge shows correctness, first-error categories, and reteaching priorities.")
    ...
    with c2:
        question_upload = st.file_uploader("Question image (optional)", type=["png", "jpg", "jpeg", "webp"], key="teacher_question_image")
        if question_upload:
            st.image(Image.open(question_upload), use_container_width=True)
    st.markdown("### Student answers")
    c1, c2 = st.columns(2)
    with c1:
        csv_upload = st.file_uploader("Upload CSV with columns: student_id, answer", type=["csv"], key="teacher_csv")
        blocks = st.text_area("Or paste answers separated by ---", height=220, key="teacher_answer_blocks")
    with c2:
        images = st.file_uploader("Or upload multiple handwritten answer images", type=["png", "jpg", "jpeg", "webp"], accept_multiple_files=True, key="teacher_answer_images")
        ...
        try:
            ...
            if not (question_text.strip() or question_upload):
                ...
                with st.spinner("Gemma is evaluating the class..."):
                    df, summary = evaluate_teacher_batch(question_text, question_upload, answers, images, level)
    ...
    st.bar_chart(df["error_category"].value_counts())
    st.dataframe(df, use_container_width=True, hide_index=True)
    csv_bytes = df.to_csv(index=False).encode("utf-8")
    c1, c2 = st.columns(2)
    with c1:
        st.download_button("Download evaluation CSV", csv_bytes, "mathbridge_teacher_evaluation.csv", "text/csv", use_container_width=True, on_click="ignore")
    with c2:
        if st.button("Prepare teacher PDF", use_container_width=True):
            ...
            st.session_state.teacher_pdf = build_pdf_bytes("MathBridge Teacher Evaluation Report", sections, df[["student_id", "status", "error_category", "first_error_step"]])
        if st.session_state.teacher_pdf:
            st.download_button("Download teacher PDF", st.session_state.teacher_pdf, "mathbridge_teacher_report.pdf", "application/pdf", use_container_width=True, on_click="ignore")
